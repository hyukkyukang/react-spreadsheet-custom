...
const EXAMPLE_DATA = Matrix.createEmpty(EXAMPLE_DATA_ROWS_COUNT, EXAMPLE_DATA_COLUMNS_COUNT);
...
describe("Selection.createEntireRows()", () => {
    ...
    test("throws for invalid start", () => {
        expect(() => Selection.createEntireRows(-1, 0)).toThrow(new Selection.InvalidIndexError("start"));
    });
    test("throws for invalid end", () => {
        expect(() => Selection.createEntireRows(0, -1)).toThrow(new Selection.InvalidIndexError("end"));
    });
});
...
describe("Selection.createEntireColumns()", () => {
    ...
    test("throws for invalid start", () => {
        expect(() => Selection.createEntireColumns(-1, 0)).toThrow(new Selection.InvalidIndexError("start"));
    });
    test("throws for invalid end", () => {
        expect(() => Selection.createEntireColumns(0, -1)).toThrow(new Selection.InvalidIndexError("end"));
    });
});
...
describe("Selection.toRange()", () => {
    const cases = [
        ...
        ["range", PointRange.create(Point.ORIGIN, Point.ORIGIN), EXAMPLE_DATA, PointRange.create(Point.ORIGIN, Point.ORIGIN)],
        [
            "entire rows",
            Selection.createEntireRows(1, 2),
            EXAMPLE_DATA,
            PointRange.create({ row: 1, column: 0 }, { row: 2, column: EXAMPLE_DATA_MAX_POINT.column }),
        ],
        [
            "entire columns",
            { type: Selection.EntireType.Column, start: 1, end: 2 },
            EXAMPLE_DATA,
            PointRange.create({ row: 0, column: 1 }, { row: EXAMPLE_DATA_MAX_POINT.row, column: 2 }),
        ],
        ["entire table", { type: Selection.EntireType.Table }, EXAMPLE_DATA, Selection.getMatrixRange(EXAMPLE_DATA)],
    ] as const;
...
describe("Selection.getSelectionFromMatrix()", () => {
    const cases = [
        ["Returns null for no selection", null, EXAMPLE_DATA, Matrix.createEmpty(0, 0)],
        ["Returns matrix for selection", PointRange.create(Point.ORIGIN, { row: 1, column: 1 }), EXAMPLE_DATA, Matrix.createEmpty(2, 2)],
    ] as const;
    test.each(cases)("%s", (name, selection, data, expected) => {
...
describe("Selection.getPoints()", () => {
    const cases = [
        ...
        ["Returns points for range", PointRange.create(Point.ORIGIN, Point.ORIGIN), [Point.ORIGIN]],
    ] as const;
...
describe("Selection.hasPoint()", () => {
    const cases = [
        ["in range", Point.ORIGIN, PointRange.create(Point.ORIGIN, Point.ORIGIN), true],
        ["not selected", EXAMPLE_NON_EXISTING_POINT, PointRange.create(Point.ORIGIN, Point.ORIGIN), false],
    ] as const;
...
describe("Selection.modifyEdge()", () => {
    const cases = [
        ...
        ["modifies entire rows", Selection.createEntireRows(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Bottom, Selection.createEntireRows(0, 1)],
        [
        ...
        ["does nothing if no active and selection", null, null, EXAMPLE_DATA, Selection.Direction.Left, null],
    ] as const;
    test.each(cases)("%s", (name, selection, active, data, edge, expected) => {
        expect(Selection.modifyEdge(selection, active, data, edge)).toEqual(expected);
    });
});
...
describe("Selection.modifyPointRangeEdge()", () => {
    ...
    test.each(cases)("%s", (name, selection, active, data, edge, expected) => {
        expect(Selection.modifyRangeEdge(selection, active, data, edge)).toEqual(expected);
    });
});

describe("Selection.modifyEntireRowsEdge()", () => {
    const cases = [
        ["modify left", Selection.createEntireRows(0, 0), { row: 0, column: 1 }, EXAMPLE_DATA, Selection.Direction.Left, Selection.createEntireRows(0, 0)],
        ["modify right", Selection.createEntireRows(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Right, Selection.createEntireRows(0, 0)],
        ["modify top", Selection.createEntireRows(1, 1), { row: 1, column: 0 }, EXAMPLE_DATA, Selection.Direction.Top, Selection.createEntireRows(0, 1)],
        ["modify top, blocked", Selection.createEntireRows(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Top, Selection.createEntireRows(0, 0)],
        ["modify top, backwards", Selection.createEntireRows(0, 1), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Top, Selection.createEntireRows(0, 0)],
        ["modify bottom", Selection.createEntireRows(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Bottom, Selection.createEntireRows(0, 1)],
        [
            "modify bottom, blocked",
            Selection.createEntireRows(EXAMPLE_DATA_MAX_POINT.row, EXAMPLE_DATA_MAX_POINT.row),
            EXAMPLE_DATA_MAX_POINT,
            EXAMPLE_DATA,
            Selection.Direction.Bottom,
            Selection.createEntireRows(EXAMPLE_DATA_MAX_POINT.row, EXAMPLE_DATA_MAX_POINT.row),
    ...
    test.each(cases)("%s", (name, selection, active, data, edge, expected) => {
        expect(Selection.modifyEntireRowsEdge(selection, active, data, edge)).toEqual(expected);
    });
});

describe("Selection.modifyEntireColumnsEdge()", () => {
    const cases = [
        ["modify top", Selection.createEntireColumns(0, 0), { row: 1, column: 0 }, EXAMPLE_DATA, Selection.Direction.Top, Selection.createEntireColumns(0, 0)],
        ["modify bottom", Selection.createEntireColumns(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Bottom, Selection.createEntireColumns(0, 0)],
        [
            "modify left",
        ...
        ["modify right", Selection.createEntireColumns(0, 0), Point.ORIGIN, EXAMPLE_DATA, Selection.Direction.Right, Selection.createEntireColumns(0, 1)],
        [
            "modify right, blocked",
            Selection.createEntireColumns(EXAMPLE_DATA_MAX_POINT.row, EXAMPLE_DATA_MAX_POINT.row),
            EXAMPLE_DATA_MAX_POINT,
            EXAMPLE_DATA,
            Selection.Direction.Right,
            Selection.createEntireColumns(EXAMPLE_DATA_MAX_POINT.row, EXAMPLE_DATA_MAX_POINT.row),
    ...
    test.each(cases)("%s", (name, selection, active, data, edge, expected) => {
        expect(Selection.modifyEntireColumnsEdge(selection, active, data, edge)).toEqual(expected);
    });
});
...
describe("Selection.hasEntireColumn()", () => {
    const cases = [
        ["returns true for entire column in selection", Selection.createEntireColumns(0, 0), 0, true],
        ["returns false for column not in entire column selection", Selection.createEntireColumns(0, 0), 1, false],
