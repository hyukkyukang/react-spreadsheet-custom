...
const reducer = createReducer(INITIAL_STATE, (builder) => {
    ...
    builder.addCase(Actions.selectEntireColumn, (state, action) => {
        ...
        return {
            ...state,
            selected: extend && active ? Selection.createEntireColumns(active.column, column) : Selection.createEntireColumns(column, column),
    ...
    builder.addCase(Actions.selectEntireRow, (state, action) => {
        ...
        return {
            ...state,
            selected: extend && active ? Selection.createEntireRows(active.row, row) : Selection.createEntireRows(row, row),
    ...
    builder.addCase(Actions.activate, (state, action) => {
        ...
        return {
            ...
            active: point,
            mode: "view",
            // isActive(state.active, point) ? "edit" : "view",
        };
    });
...
const shiftKeyDownHandlers: KeyDownHandlers = {
    ArrowUp: (state) => ({
        ...state,
        selected: Selection.modifyEdge(state.selected, state.active, state.data, Selection.Direction.Top),
    }),
    ArrowDown: (state) => ({
        ...state,
        selected: Selection.modifyEdge(state.selected, state.active, state.data, Selection.Direction.Bottom),
    }),
    ArrowLeft: (state) => ({
        ...state,
        selected: Selection.modifyEdge(state.selected, state.active, state.data, Selection.Direction.Left),
    }),
    ArrowRight: (state) => ({
        ...state,
        selected: Selection.modifyEdge(state.selected, state.active, state.data, Selection.Direction.Right),
    }),
    Tab: go(0, -1),
};
