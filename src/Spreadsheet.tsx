...
import DefaultCornerIndicator, { enhance as enhanceCornerIndicator } from "./CornerIndicator";
import DefaultColumnIndicator, { enhance as enhanceColumnIndicator } from "./ColumnIndicator";
import DefaultRowIndicator, { enhance as enhanceRowIndicator } from "./RowIndicator";
 ...
 */
const Spreadsheet = <CellType extends Types.CellBase>(props: Props<CellType>): React.ReactElement => {
    const {
        className,
        darkMode,
        columnLabels,
    ...
    const paste = React.useCallback((data) => dispatch(Actions.paste(data)), [dispatch]);
    ...
    const setData = React.useCallback((data) => dispatch(Actions.setData(data)), [dispatch]);
    const blur = React.useCallback(() => dispatch(Actions.blur()), [dispatch]);
    const hasPointSelected: boolean = useSelector((state) => Selection.hasPointSelected(state.selected));
    ...
    const handleMouseLeave = React.useCallback(() => {
        ...
        if (hasPointSelected) {
            ...
            // set all activation to null
            dispatch(Actions.activate(Point.NULLPOINT));
        }
    }, [dispatch, hasPointSelected]);
    ...
    const CornerIndicator = React.useMemo(() => enhanceCornerIndicator(props.CornerIndicator || DefaultCornerIndicator), [props.CornerIndicator]);
            ...
            <Table columns={size.columns} hideColumnIndicators={hideColumnIndicators}>
                ...
                {range(size.rows).map((rowNumber) => (
                    <Row key={rowNumber} row={rowNumber}>
                        {!hideRowIndicators &&
                            (rowLabels ? (
                                <RowIndicator key={rowNumber} row={rowNumber} label={rowNumber in rowLabels ? rowLabels[rowNumber] : null} />
                            ) : (
    ...
    const rootNode = React.useMemo(
        ...
        [className, darkMode, onKeyPress, handleKeyDown, handleMouseMove, handleBlur, tableNode]
    );
