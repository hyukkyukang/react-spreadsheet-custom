...
import reducer, { INITIAL_STATE, hasKeyDownHandler, isActiveReadOnly } from "./reducer";
...
describe("reducer", () => {
    const cases: Array<[name: string, state: Types.StoreState, action: Action<unknown>, expected: Types.StoreState]> = [
        ["setData", INITIAL_STATE, Actions.setData(EXAMPLE_DATA), { ...INITIAL_STATE, data: EXAMPLE_DATA }],
        [
            "select",
        ...
        [
            "select multiple entire rows forwards",
            {
                ...INITIAL_STATE,
                active: Point.ORIGIN,
                selected: Selection.createEntireRows(Point.ORIGIN.row, Point.ORIGIN.row),
            },
        ...
        [
            "select multiple entire columns",
            {
                ...INITIAL_STATE,
                active: Point.ORIGIN,
                selected: Selection.createEntireColumns(Point.ORIGIN.column, Point.ORIGIN.column),
            },
            Actions.selectEntireColumn(EXAMPLE_COLUMN, true),
            {
                ...INITIAL_STATE,
                active: Point.ORIGIN,
                selected: Selection.createEntireColumns(Point.ORIGIN.column, EXAMPLE_COLUMN),
            },
        ],
        [
            "activate",
        ...
        [
            "setCellData",
            INITIAL_STATE,
            Actions.setCellData(Point.ORIGIN, EXAMPLE_CELL, MOCK_GET_BINDINGS_FOR_CELL),
            {
                ...INITIAL_STATE,
        ...
        ["dragStart", INITIAL_STATE, Actions.dragStart(), { ...INITIAL_STATE, dragging: true }],
        ["dragEnd", { ...INITIAL_STATE, dragging: true }, Actions.dragEnd(), INITIAL_STATE],
    ];
...
describe("hasKeyDownHandler", () => {
    const cases = [
        ...
        ["returns true for handled key in view unhandled in edit", INITIAL_STATE, "Backspace", true],
        ["returns false for unhandled key in edit", EDIT_STATE, "Delete", false],
        ["returns true for handled key in view unhandled in edit", INITIAL_STATE, "Delete", true],
    ] as const;
    test.each(cases)("%s", (name, state, key, expected) => {
        expect(hasKeyDownHandler(state, { key } as React.KeyboardEvent)).toBe(expected);
    });
});
...
describe("isActiveReadOnly", () => {
    const cases = [
        ...
        ["returns false if active is not read only", { ...INITIAL_STATE, active: Point.ORIGIN }, false],
        [
            "returns true if active is read only",
            {
                ...INITIAL_STATE,
                data: [[{ readOnly: true, value: undefined }]] as Matrix.Matrix<Types.CellBase>,
                active: Point.ORIGIN,
            },
            true,
        ],
