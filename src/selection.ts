...
export type Selection = PointRange.PointRange | EntireRows | EntireColumns | EntireTable | null;
...
export function toRange(selection: Selection, data: Matrix.Matrix<unknown>): PointRange.PointRange | null {
    if (selection === null) {
        return null;
    }
    ...
    switch (selection.type) {
        case EntireType.Row: {
            ...
            return PointRange.create({ row: selection.start, column: -1 }, { row: selection.end, column: -1 });
        }
        case EntireType.Column: {
            const max = Matrix.maxPoint(data);
            return PointRange.create({ row: -1, column: selection.start }, { row: -1, column: selection.end });
        }
...
export function size(selection: Selection, data: Matrix.Matrix<unknown>): number {
    const range = toRange(selection, data);
    return range ? PointRange.size(range) : 0;
}

export function hasPointSelected(selection: Selection): boolean {
    return selection !== null && PointRange.is(selection) && selection.start.column !== -1;
}

export function getSelectedPoint(selection: Selection): Point.Point | null {
    if (hasPointSelected(selection)) {
        if (PointRange.is(selection)) {
...
export function hasPoint(selection: Selection, data: Matrix.Matrix<unknown>, point: Point.Point): boolean {
    const range = toRange(selection, data);
    return range !== null && PointRange.has(range, point);
}

/** Return whether the given header is selected in given selection */
export function hasHeaderSelected(selection: Selection, column: number): boolean {
    if (selection === null || !PointRange.is(selection)) {
        return false;
    }
    return selection.start.row === -1 && selection.end.row === -1 && column === selection.start.column;
}
...
export function normalize(selection: Selection, data: Matrix.Matrix<unknown>): Selection {
    if (selection) {
        if (PointRange.is(selection)) {
...
export function normalizeRange(range: PointRange.PointRange, data: Matrix.Matrix<unknown>): PointRange.PointRange {
...
export function normalizeEntireRows(selection: EntireRows, data: Matrix.Matrix<unknown>): EntireRows {
...
export function normalizeEntireColumns(selection: EntireColumns, data: Matrix.Matrix<unknown>): EntireColumns {
...
/** Get selected points */
export function getPoints(selection: Selection, data: Matrix.Matrix<unknown>): Point.Point[] {
    const range = toRange(selection, data);
    return range ? Array.from(PointRange.iterate(range)) : [];
}

/** Get given selection from given data */
export function getSelectionFromMatrix<T>(selection: Selection, data: Matrix.Matrix<T>): Matrix.Matrix<T> {
    const range = toRange(selection, data);
...
export function modifyEdge(selection: Selection, active: Point.Point | null, data: Matrix.Matrix<unknown>, edge: Direction): Selection {
    if (active && selection) {
        if (PointRange.is(selection)) {
...
export function modifyEntireRowsEdge(selection: EntireRows, active: Point.Point, data: Matrix.Matrix<unknown>, edge: Direction): EntireRows {
...
/** Modify given edge of given entire rows selection according to given active point and active matrix */
export function modifyEntireColumnsEdge(selection: EntireColumns, active: Point.Point, data: Matrix.Matrix<unknown>, edge: Direction): EntireColumns {
...
export function modifyRangeEdge(range: PointRange.PointRange, active: Point.Point, data: Matrix.Matrix<unknown>, edge: Direction): PointRange.PointRange {
...
export function getMatrixRange(data: Matrix.Matrix<unknown>): PointRange.PointRange {
...
export function getRangeFromMatrix<T>(range: PointRange.PointRange, matrix: Matrix.Matrix<T>): Matrix.Matrix<T> {
